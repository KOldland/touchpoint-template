 ...
 * Integrates with Dual GPT plugin for API key management.
 *
 * @package KHM\GEO
 */
...
class LLMClient {
     ...
     * 3. KHM GEO option (khm_geo_openai_api_key)
     * 4. WordPress constant OPENAI_API_KEY
     *
     * @return string|null
     */
    private function get_api_key() {
        ...
        // 3. KHM GEO specific option
        $khm_key = get_option( 'khm_geo_openai_api_key' );
        if ( $khm_key ) {
            $this->api_key_source = 'khm_option';
            return $khm_key;
        }

        // 4. WordPress constant
    ...
    private function get_model() {
        ...
        $env_model = getenv( 'KHM_GEO_LLM_MODEL' );
        if ( $env_model ) {
            return $env_model;
        }

        if ( defined( 'KHM_GEO_LLM_MODEL' ) ) {
            return KHM_GEO_LLM_MODEL;
        }

        // Check option
        $option_model = get_option( 'khm_geo_llm_model' );
        if ( $option_model ) {
            return $option_model;
        }
    ...
    public function call( $system_prompt, $user_prompt, $options = array() ) {
        if ( ! $this->api_key ) {
            return new \WP_Error(
                'no_api_key',
                __( 'OpenAI API key not configured. Set it in Dual GPT settings or as OPENAI_API_KEY environment variable.', 'khm-membership' )
            );
        }
    ...
    private function make_request_with_retry( $endpoint, $data ) {
        ...
        for ( $attempt = 0; $attempt < $this->max_retries; $attempt++ ) {
            ...
            error_log( sprintf(
                '[KHM GEO LLMClient] Attempt %d failed: %s. Retrying...',
                $attempt + 1,
    ...
    private function make_request( $endpoint, $data ) {
        ...
        if ( $code === 401 ) {
            return new \WP_Error( 'invalid_api_key', __( 'Invalid OpenAI API key', 'khm-membership' ) );
        }

        if ( $code === 429 ) {
            return new \WP_Error( 'rate_limited', __( 'OpenAI rate limit exceeded', 'khm-membership' ) );
        }

        if ( $code >= 500 ) {
            return new \WP_Error( 'server_error', __( 'OpenAI server error', 'khm-membership' ) );
        }

        if ( $code !== 200 ) {
            $message = $data['error']['message'] ?? __( 'Unknown API error', 'khm-membership' );
